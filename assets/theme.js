...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
      ...
      //krack
//       $(".product-single__thumbnails li > a").css('display', 'none'); 
// $(".product-single__thumbnails li").find('.filter-All, .filter-' + variant.option1.replace(' ', '-').replace('/', '-')).css('display', 'block');
...
function nextDate(dayIndex) {
    var today = new Date();
    today.setDate(today.getDate() + (dayIndex - 0 - today.getDay() + 7) % 7 + 0);
    return today;
}
// console.log("Offer Ends this Monday on "+nextDate(1).toLocaleString('en-us',{month:'long', year:'numeric', day:'numeric'}));
   
   offerTag = $('body').find('p.offerEndDate');
   
   if(offerTag.length){
   	offerTag.text("Sale Ends this Sunday - "+nextDate(0).toLocaleString('en-us',{month:'long', year:'numeric', day:'numeric'}));
   }
   
  	var tabTitle = '<ul>';
  jQuery('.faq > div.tab-content').each(function(i, item){
      jQuery(this).attr('id', "tabs-" + i);
      var titletext = jQuery(this).find('h3').text();
//       var titlelist = '<li><a href="#tabs-' + i + '">' + titletext + '</a></li>'
//       tabTitle += titlelist ;
...
//      $('.faq .tab-content h3').click(function(){
//        var myChild $(this).parent().find('p')
//        active.removeClass('active');
//         content.hide();
//         active = $(this);
//         content = $($(this).attr('href'));
...
// console.log("Offer Ends this Monday on "+nextDate(1).toLocaleString('en-us',{month:'long', year:'numeric', day:'numeric'}));
   ...
   offerTag = $('body').find('p.offerEndDate2');
...
window.onload = function () {
  ...
  var slider = null;
  if(jQuery) {
    var sh = $('#lightSlider-s')

    var sliderNew = sh.lightSlider({
              adaptiveHeight:true,
      //         item:1,
      //         slideMargin:0,
      //         loop:true,
...
//     console.log
//   }
  function addBringThemIn(x){
  	$('#shopify-section-1564034326803').find(x).addClass('bringThemIn')
  }
   function addBringThemIn2(x){
  	$('body').find('#shopify-section-1564117322086').addClass('bringThemIn');
  }
  function addBringThemIn3(){
  	$('body').find('#shopify-section-1551078450560').addClass('bringThemIn');
    $('#shopify-section-1550650210783').addClass('bringThemIn');
  }
   $('.loadingScreen').fadeOut(function(){
  	setTimeout(function(){ addBringThemIn('h4'); },200);
    setTimeout(function(){ addBringThemIn('p'); },300);
     setTimeout(function(){ addBringThemIn2('img'); },400);
     setTimeout(function(){ addBringThemIn3(); },600);
...
//   });
  ...
  $('.custom-content .custom__item img').click(function(){
...
//   catchPricethree = catchPricethree.find('li:nth-child(3)');
  
//   catchPricethree = catchPricethree.find('div.price__sale');
//   catchPricethree = catchPricethree.find('span.price-item');
//   catchPricethree.text('Starting from $2999.99');
  // catchprice 3 changes price for the 9th item on the collection page. this was used to make Tuscany Console reflect a 3 seater price instead of a 2 seater which is standard
  
//   var catchPricefour = $('body').find('#shopify-section-collections-all');
  
//   catchPricefour = catchPricefour.find('li:nth-child(6)');
  
//   catchPricefour = catchPricefour.find('div.price__sale');
//   catchPricefour = catchPricefour.find('span.price-item');
//   catchPricefour.text('Starting from $3299.99');
    // catchprice 4 changes price for the 8th item on the collection page. this was used to make Oxford Console reflect a 3 seater price instead of a 2 seater which is standard

  minifyObjectHolder = $('body').find('#product-holder');
  minifyObject = $('body').find('#product-holder .product-single__photos');
  minify3dObject = $('body').find('#product-holder .iframe-container');
  hideMobTopCartButton = $('body').find('.mob-cart-button-holder');
  hideSwipeButton = $('body').find('#swipe-indicator');

  
//   window.onscroll = function() {minifier()};
document.addEventListener('scroll', minifier, {passive: true});
  function minifier() {
    ...
    if (window.matchMedia("(orientation: portrait)").matches) {
      ...
      if (document.body.scrollTop > 50 || document.documentElement.scrollTop > 150) {
        ...
        if(minifyObject.length){
          minifyObject.addClass('minify-it');
        }
        if(minifyObjectHolder.length){
          minifyObjectHolder.addClass('minify-it-index');
        }
        if(minify3dObject.length){
          minify3dObject.addClass('minify-it-new');
        }
        if(hideMobTopCartButton.length){
          hideMobTopCartButton.addClass('hide-minify');
        }
        if(hideSwipeButton.length){
          hideSwipeButton.addClass('hide-minify');
        }

      } else {
        if(minifyObjectHolder.length){
          minifyObjectHolder.removeClass('minify-it-index');
        }
        if(minifyObject.length){
          minifyObject.removeClass('minify-it');
        }
        if(minify3dObject.length){
          minify3dObject.removeClass('minify-it-new');
        }
        if(hideMobTopCartButton.length){
          hideMobTopCartButton.removeClass('hide-minify');
        }

        //     console.log('on top now');
...
$(document).ready(function() {
  $("#wine-glass-caddy").click(function() {
    $("#overlay").show();
  });
});
